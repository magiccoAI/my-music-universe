...
const Aurora = ({ position = [0, 10, -50], scale = [1, 1, 1], isMobile = false }) => {
  ...
  return (
    <group position={position} scale={scale}>
      ...
      <mesh position={[0, 0, 0]} rotation={[0, 0, 0]}>
       ...
       {/* 移动端只渲染一层极光，或者根据性能需求决定。
           为了保证“极光”的视觉特征，这里移动端仍然保留两层，但顶点数大大减少。
           如果非常卡顿，可以移除下面这个 mesh */}
       <mesh position={[0, -5, -10]} rotation={[0, 0.1, 0]}>
        <planeGeometry args={isMobile ? [120, 50, 24, 24] : [120, 50, 64, 64]} />
        <shaderMaterial
          ref={materialRef2}
          uniforms={{
             ...uniforms,
             uTime: { value: 0 }, // 确保这里也有 uTime 对象结构，虽然 ...uniforms 已经包含了，但为了保险起见
