      ...
      if (reelRef.current) {
        ...
        for (let i = 0; i < 20; i++) {
          ...
          albumElement.innerHTML = `
            <img src="${process.env.PUBLIC_URL}/optimized-images/${randomAlbum.cover.split('/').pop().replace(/\.(png|jpg|jpeg)$/i, '')}.webp" alt="${randomAlbum.title}" class="album-cover" onerror="this.onerror=null;console.error('Image failed to load:', this.src)" onload="console.log('Image loaded:', this.src)">
    ...
    reelRefs.forEach((reelRef, index) => {
      if (reelRef.current) {
        ...
        for (let i = 0; i < itemsInReel; i++) {
          ...
          albumElement.className = 'album-item';
          albumElement.innerHTML = `
            <img src="${process.env.PUBLIC_URL}/optimized-images/${albumToDisplay.cover.split('/').pop().replace(/\.(png|jpg|jpeg)$/i, '')}.webp" alt="${albumToDisplay.title}" class="album-cover" onerror="this.onerror=null;console.error('Image failed to load:', this.src)" onLoad="console.log('Image loaded:', this.src)">
            <div class="album-info">
